...
exports.create = (req, res) => {
    ...
    const usuario = new Usuario({
      ...
      descripcion: req.body.estado,
      precio: req.body.plan
    });
...
exports.list = (req, res) => {
  ...
  exports.getId = (req, res) => {
    Usuario.findById(req.params.usuario_id, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
            message: `usuario no encontrado id ${req.params.usuario_id}.`
          });
        } else {
          res.status(500).send({
            message: "error al buscar id " + req.params.usuario_id
          });
        }
      } else res.send(data);
    });
      ...
      (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            ...
            res.status(500).send({
              message: "error al actualizar usuario id " + req.params.usuario_id
            });
          }
        } else res.send(data);
      }
